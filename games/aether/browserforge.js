...
async function renderModpacks() {
  ...
  if (modpacks.length === 0) {
    mpList.innerHTML = '<p style="color:#aaa; font-style:italic;">No modpacks found. Create one to get started!</p>';
    return;
  }
...
function openModal(isEdit = false) {
  document.getElementById('modal-title').textContent = isEdit ? 'Edit Modpack' : 'Create Modpack';
...
function editModpack(mp) {
  ...
  document.getElementById('mp-opengl').checked = mp.config?.opengl !== undefined ? mp.config.opengl : false;
  document.getElementById('mp-gc').value = mp.config?.gc || '';
  document.getElementById('mp-aggressive').checked = mp.config?.aggressive || false;
...
async function playModpack(mp) {
  ...
  if (mp.config) {
    document.getElementById('memory-select').value = mp.config.memory || 'default';
    document.getElementById('opengl-toggle').checked = mp.config.opengl !== undefined ? mp.config.opengl : false;
    document.getElementById('gc-select').value = mp.config.gc || '';
    document.getElementById('aggressive-opts').checked = mp.config.aggressive || false;
...
function syncVersionConfig() {
  ...
  document.getElementById('res-width').value = '854';
  document.getElementById('res-height').value = '480';
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    } else if (arg === '-XX:+UseG1GC' || arg === '-XX:+UseParallelGC' || arg === '-XX:+UseConcMarkSweepGC') {
...
function updateArgsPreview() {
  ...
  const managedPrefixes = ['-Xmx', '-XX:+Use', '-XX:+AggressiveOpts', '-Dsun.java2d.opengl=', '--username', '--width', '--height'];
  ...
  customArgsInput.value = (managed.join(' ') + " " + manualFlags.join(' ')).trim();
}
...
['memory-select', 'opengl-toggle', 'gc-select', 'aggressive-opts', 'username', 'res-width', 'res-height'].forEach(id => {
...
playBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (uploadedMods.length > 0 && (versionType === 'zip' || versionType === 'jar-zip')) {
    ...
    if (customArgsInput && customArgsInput.value.trim()) {
      ...
      for (let i = 0; i < allTokens.length; i++) {
        const t = allTokens[i];
        if (t.startsWith('-D') || t.startsWith('-X') || t.startsWith('-XX') || t.startsWith('_JAVA_')) {
    ...
    let libs = { "libGL.so.1": "/app/libraries/gl4es.wasm" };
